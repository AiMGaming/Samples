...
// 15/10/2014 - Added Powered by DroidScript splash screen and released to Play Store
//
...
function OnStart()
{
    ...
    laySplashScreen = app.CreateLayout("Linear", "Vertical, FillXY, VCenter");
    {
        laySplashScreen.SetBackColor("#8EA22A");
        
        var spaceShooterText = app.CreateText("Space Shooter Demo", 0.9, 0.1, "AutoScale");
        spaceShooterText.SetMargins(0, 0.4, 0, 0);
        spaceShooterText.SetTextColor("#FFFFFF");
        spaceShooterText.SetTextSize(64);
        spaceShooterText.SetTextShadow(5, 2, 2, "#66000000");
        laySplashScreen.AddChild(spaceShooterText);
        
        var poweredByText = app.CreateText("Powered by DroidScript", 0.7, 0.1, "AutoScale");
        poweredByText.SetMargins(0, 0.1, 0, 0);
        poweredByText.SetTextColor("#FFFFFF");
        poweredByText.SetTextSize(64);
        poweredByText.SetTextShadow(5, 2, 2, "#66000000");
        laySplashScreen.AddChild(poweredByText);
        
        var layLower = app.CreateLayout("Linear", "FillXY, Right, Bottom");
        {
            var bgImage = app.CreateImage("Img/backgroundlogo.png", 0.3, -1);
            layLower.AddChild(bgImage);
        }
        laySplashScreen.AddChild(layLower);
    }
    layMain.AddChild(laySplashScreen);
    ...
    // Keep the splashscreen visible for 2 seconds
    startGameTimer = setTimeout(StartGame, 2000);
...
function OnPause() 
{ 
    // If the Home button is pressed while the splashscreen is showing, make sure
    // we prevent StartGame being called while hidden.
    clearTimeout(startGameTimer);
    
    // Stop rendering when hidden
    clearInterval(renderFrameTimer);
...
function OnResume() 
{ 
    // If GameStarted has been called, start the music again and rendering.
    // Otherwise start the game as the Splash Screen will still be visible.
    if(gameStarted)
    {
        ...
        renderFrameTimer = setInterval(RenderFrame, 1000/30);
    }
    else
    {
        // Leave the splash screen visible for 2 seconds, then start the game
        startGameTimer = setTimeout(StartGame, 2000);
    }
}
...
function StartGame()
{
    ...
    laySplashScreen.Animate("SlideToLeft", OnSplashScreenHidden);
}
...
function OnSplashScreenHidden()
{
    ...
    layScore.SetVisibility("Show");
    
    // Until gameStarted = true, OnTouch will be disabled.
    gameStarted = true;
}
...
{   
    // Ignore OnTouch while the splash screen is visible
    if(!gameStarted)
    {
        return;
    }
